...
func _init()->void:
	connect(&"fade_start", Callable(self, &"set_fading").bind(true))
	connect(&"fade_end", Callable(self, &"set_fading").bind(false))
